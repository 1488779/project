...
const CHATS = [
  {
    ...
    unread: 0,
    messages: [
      { id: 1, from: "them", text: "Добрый день! Могу ли я взять задачу по доставке корма?", time: "14:20" },
      { id: 2, from: "me", text: "Конечно! Жду вас завтра в 14:00 по адресу ул. Мира, 28", time: "14:22" },
      { id: 3, from: "them", text: "Спасибо за помощь!", time: "14:35" },
    ],
  },
  {
    id: 2,
    name: 'Приют "Добрые руки"',
    lastMessage: "Когда сможете подъехать?",
    time: "13:20",
    unread: 2,
    messages: [
      { id: 1, from: "them", text: "Добрый день! Отлично, когда вы сможете приехать?", time: "13:10" },
      { id: 2, from: "me", text: "Я смогу завтра в 14:00", time: "13:15" },
      { id: 3, from: "them", text: "Прекрасно! Жду вас по адресу ул. Мира, 28", time: "13:18" },
      { id: 4, from: "them", text: "Когда сможете подъехать?", time: "13:20" },
    ],
  },
  {
    id: 3,
    ...
    unread: 0,
    messages: [
      { id: 1, from: "them", text: "Здравствуйте! Хочу взять кота на передержку", time: "Вчера" },
      { id: 2, from: "me", text: "Замечательно! Расскажите подробнее о себе", time: "Вчера" },
      { id: 3, from: "them", text: "Отлично, договорились", time: "Вчера" },
    ],
  },
];
...
export default function ChatPage() {
  ...
  const sendMessage = (text) => {
    if (!text.trim()) return;
    const newMsg = { id: Date.now(), from: "me", text: text.trim(), time: new Date().toLocaleTimeString("ru-RU", { hour: "2-digit", minute: "2-digit" }) };
              ...
              {CHATS.map((chat) => (
                <button
                  ...
                  className={`w-full text-left px-5 py-4 flex items-center gap-3 hover:bg-[#f5f5f5] transition-colors border-l-2 ${
                    selectedChat.id === chat.id ? "bg-[#f0f7f1] border-[#3a7d44]" : "border-transparent"
                  }`}
                >
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-3">
              {(messages[selectedChat.id] || []).map((msg) => (
                <div key={msg.id} className={`flex ${msg.from === "me" ? "justify-end" : "justify-start"}`}>
                  ...
                  >
                    <p>{msg.text}</p>
                    <p className={`text-xs mt-1 ${msg.from === "me" ? "text-green-200 text-right" : "text-[#9e9e9e]"}`}>{msg.time}</p>
                  </div>
                </div>
              ))}
